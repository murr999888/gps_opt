...
Ext.define('Opt.view.Map', {
	extend: 'Ext.panel.Panel',
	xtype: 'leafletmap',
	referenceHolder: true,
	polygonGeofenceStrumok: null,
	config: {
	...
	renderMap: function () {
		...
		if (ll && !element.dom._leaflet) {
			...
			if (!mapOptions.hasOwnProperty('center')) {
				mapOptions.center = new ll.LatLng(37.773972, -122.431297);
			}
...
*/
	...
	setAdditionalControls: function setAddControls() {
...
*/
		...
		bingAerial = L.tileLayer.bing({bingMapsKey:'your_api_key_here', imagerySet:'Aerial'});
	...
	setGeofenceStrumok: function () {
		if (this.polygonGeofenceStrumok != null) {
			this.removeLayer(polygonGeofenceStrumok);
			this.polygonGeofenceStrumok = null;
		}

		var pts = geofence_strumok;
		this.polygonGeofenceStrumok = L.polygon(pts, {
			color: 'blue',
			opacity: 0.2,
			fillOpacity: 0.1,
			weight: 1
		...
		this.polygonGeofenceStrumok.bindPopup("Домашняя геозона Струмок");
		this.polygonGeofenceStrumok.addTo(this.map);

		strumokGeoJson = L.geoJson(this.polygonGeofenceStrumok.toGeoJSON());
	},
