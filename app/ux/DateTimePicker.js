...
Ext.define('Opt.ux.DateTimePicker', {
	...
	me.secondSlider.addListener('change', me.changeTimeValue, me);
    },
    ...
    changeTimeValue: function(slider, e, eOpts) {
        ...
        label.setText(hourPrefix + hourDisplay + ':' + minutePrefix + me.minuteSlider.getValue() + ':' + me.secondSlider.getValue()+ (timeSufix || ''));

        if (me.pickerField && (pickerValue = me.pickerField.getValue())) {
		var setHMS ='';
		if (slider == me.hourSlider) {
			setHMS = 'setHours';
		}

		if (slider == me.minuteSlider) {
			setHMS = 'setMinutes';
		}		

		if (slider == me.secondSlider) {
			setHMS = 'setSeconds';
		}

            auxValue = new Date(pickerValue[setHMS](slider.getValue()));
