	...
	if ($obj)  {
		if (is_array($obj)) {
			foreach ($obj as $result) {
				$strq = "INSERT ?n (name, speed, begin_lat, begin_lon, end_lat, end_lon, geometry) VALUES (?s, ?s, ?s, ?s, ?s, ?s, ?s)";
				$DB->query($strq, 'traffic', $result["name"], $result["speed"], $result["begin_lat"], $result["begin_lon"], $result["end_lat"], $result["end_lon"], $result["geometry"]);
				$id = $DB->insertId();
